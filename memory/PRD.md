...
### P0 (Completed)
- [x] **Catalogue Marché/Supermarché éditable (93 produits FCFA Bénin)** : Nouveau référentiel `quick_products` seedé automatiquement au démarrage avec ~93 produits courants (Boissons, Viandes & Poissons, Légumes, Fruits, Épicerie, Décoration & Fête, Vaisselle & Service, Services & Animation, Glace & Combustible). Source "Marché / Supermarché" ajoutée au Simulateur Locations → grille violette cliquable. Bouton "Gérer" → modal CRUD (ajout/édition/suppression) en mode admin. 4/4 tests quick_products + 4/4 simulator = 8/8 PASSED. (22/05/2026)
- [x] **Simulateur de devis Locations** : Nouveau sous-onglet "Simulateur de devis" dans Locations. Ajout d'articles libres + sélection depuis catalogue Stock + Caisse (auto-fill prix d'achat). Marge configurable en % ou montant fixe. Affichage simultané : Prix de revient + Prix de vente global + Par personne. CRUD complet (sauvegarde, chargement, modification, suppression). 4/4 tests PASSED. (22/05/2026)
- [x] **Facture créée uniquement à l'impression du bon client + Tracking tables fixé** : "ENVOYER LA COMMANDE" stocke les items + status `ready_to_invoice` + imprime les bons cuisine/bar/jeux (PAS de facture en BDD). Nouveau bouton "Imprimer le bon client" sur table en `ready_to_invoice` → crée la facture + imprime le ticket. Tracking : timer figé à `last_order_sent_at` (ready_to_invoice) ou `invoice_created_at` (invoiced). Auto-libération des tables invoiced > 30 min. 5/5 tests PASSED, 29/29 tests cumulés. (22/05/2026)
