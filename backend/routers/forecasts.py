...
async def analyze_single_request(
    ...
    available: float,
    id_field: str = "id",
    self_ref: str = "expense_id",
) -> dict:
    """Compute the full analysis (duplicates/intra/stock/purchases/treasury)
    for a single item-bearing request (expense OR need).

    Returns a dict with the same schema used by /expenses/analysis.
    `self_ref` is the key under which the request id is echoed (expense_id | need_id).
    """
    eid = e.get(id_field)
    ...
    for other in recent_requests:
        if other.get("id") == eid or other.get("status") in ("cancelled", "annule"):
            continue
        ...
        for oit in other.get("items") or []:
            ...
            for cur in cur_items_norm:
                if _items_match(cur["norm"], o_norm):
                    _add_dup_item(
                        cur_raw=cur["raw"], match_raw=o_raw, source="request",
                        source_label=o_desc, source_date=o_date,
        ...
        for pi in p.get("items", []) or []:
            ...
            for cur in cur_items_norm:
                if _items_match(cur["norm"], p_norm):
                    _add_dup_item(
                        cur_raw=cur["raw"], match_raw=p_raw, source="purchase",
                        source_label=f"Achat • {p_sup}", source_date=p_date,
    ...
    # Duplicates aggregated (by other request)
    duplicates = []
    for other in recent_requests:
        if other.get("id") == eid or other.get("status") in ("cancelled", "annule"):
            continue
    ...
    for item in items_to_check:
        ...
        for sp in stock_products:
            ...
            if _items_match(iname_norm, sp_norm):
                last_entry = await db_ref.stock_movements.find_one({
    ...
    ratio = (amount / available * 100) if (available and available > 0 and amount > 0) else None
    if amount <= 0:
        level = "low"
    elif ratio is None:
        level = "critical"
    ...
    return {
        ...
        "treasury_impact": {
            ...
            "level": level,
            "would_remain": available - amount if available else -amount,
        }
    }
    ...
    try:
        ...
        for e in exp:
            analyses.append(await analyze_single_request(
                db, e, recent, recent_purchases, stock_products, available,
                id_field="id", self_ref="expense_id",
            ))
